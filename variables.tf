...
variable "location" {
  type        = string
  description = "The Azure region for data collection resources, required even when using an existing Data Collection Rule."
  nullable    = false

  validation {
    condition     = var.create_data_collection_resources == true ? var.location != "" : true
    error_message = "You must provide a non-empty 'location' when 'create_data_collection_resources' is set to true."
  }
}
